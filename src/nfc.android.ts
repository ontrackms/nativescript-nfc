...
import { AndroidApplication, Application, Utils, AndroidActivityEventData, AndroidActivityNewIntentEventData, Trace } from "@nativescript/core";
...
export class Nfc implements NfcApi {
  ...
  constructor() {
    ...
    if (Nfc.firstInstance) {
      ...
      Application.android.on(AndroidApplication.activityCreatedEvent, (args: AndroidActivityEventData) => {
        this.initNfcAdapter();
      });
      ...
      Application.android.on(AndroidApplication.activityNewIntentEvent, (args: AndroidActivityNewIntentEventData) => {
  ...
  public available(): Promise<boolean> {
    return new Promise((resolve, reject) => {
      let nfcAdapter = android.nfc.NfcAdapter.getDefaultAdapter(Utils.android.getApplicationContext());
  ...
  public setOnTagDiscoveredListener(callback: (data: NfcTagData) => void): Promise<void> {
    return new Promise((resolve, reject) => {
      onTagDiscoveredListener = Utils.mainThreadify(callback);
      resolve();
    });
  }
  ...
  public setOnNdefDiscoveredListener(callback: (data: NfcNdefData) => void, options?: NdefListenerOptions): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      onNdefDiscoveredListener = Utils.mainThreadify(callback);
      resolve();
    });
  }
    ...
    if (!this.created) {
      ...
      if (activity) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.S) {
          mutability = android.app.PendingIntent.FLAG_MUTABLE;
        }
        this.pendingIntent = android.app.PendingIntent.getActivity(activity, 0, this.intent, mutability);
